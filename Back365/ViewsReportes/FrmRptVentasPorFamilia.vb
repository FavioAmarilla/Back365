...
Public Class FrmRptVentasPorFamilia

    Dim reporte As New ContReportes
    Dim dtVentas As New DataTable
    Dim titulo As String = "Reporte de Ventas p/ Familia"

    Dim idUnidadOperat As Integer = 0
    Dim idFamilia As Integer = 0
    Dim idSucursal As Integer = 0

    Private Sub FrmRptVentasPorFamilia_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub vistas(vr As String)
        If vr = "UO" Then
            ...
            frm.setFrmActivo("FrmRptVentasPorFamilia")
            frm.ShowDialog()
        End If
    ...
    Sub consultar()
        ...
        Dim sql As String = "SELECT * FROM(SELECT ID_LINEA, FAMILIA, ID_PROVEEDOR, PROVEEDOR, CANTIDAD, COSTO_UNITARIO, PRECIO_VENTA, 
                            ROUND(((PRECIO_VENTA / (SELECT SUM(VIT.PRECIO_UNITARIO * VIT.CANTIDAD)
                                            ...
                                            WHERE  PR_PRODUCTOS.ID_LINEA=" & idFamilia.ToString & " AND VTA_COMPROBANTES.FEC_ANULACION IS NULL 
                                            AND VTA_COMPROBANTES.ID_UNIDAD=" & idUnidadOperat.ToString & " AND VTA_COMPROBANTES.ID_SITIO=" & idSucursal.ToString & "
                            ...
                            FROM (
                                    ...
                                    COALESCE(CP_PROVEEDORES.IDENTIFICADOR, 0) AS ID_PROVEEDOR, COALESCE(CP_PROVEEDORES.DENOMINACION, 'NO DEFINIDO') AS PROVEEDOR, 
                                    ...
                                    WHERE ID_LINEA=" & idFamilia.ToString & " AND VTA_COMPROBANTES.FEC_ANULACION IS NULL 
                                    AND VTA_COMPROBANTES.ID_UNIDAD=" & idUnidadOperat.ToString & " AND VTA_COMPROBANTES.ID_SITIO=" & idSucursal.ToString & "
                                    ...
                                    CP_PROVEEDORES.IDENTIFICADOR , CP_PROVEEDORES.DENOMINACION
    ...
    Sub getItems(idProveedor As String)
        Dim dt As New DataTable
        Dim sql As String = ""
        Dim desde As String = txtFechaDesde.Text
        Dim hasta As String = txtFechaHasta.Text

        sql = "SELECT CODIGO_BARRAS, PRODUCTO, PROVEEDOR, CANTIDAD_VENDIDA, TOTAL_COSTO, TOTAL_VENTA, 
                    CASE 
                      WHEN CANTIDAD_VENDIDA > 0 THEN 
                        CASE 
                          WHEN TOTAL_COSTO > 0 THEN 
                            ROUND((((TOTAL_VENTA / TOTAL_COSTO) * 100)-100), 2)
                          ELSE 0 
                        END
                      ELSE 0 
                    END AS RENTABILIDAD  
                FROM (
                    SELECT PR_PRODUCTOS.CODIGO_BARRAS, PR_PRODUCTOS.DESCRIPCION AS PRODUCTO, CP_PROVEEDORES.DENOMINACION AS PROVEEDOR, SUM(IT.CANTIDAD) AS CANTIDAD_VENDIDA, 
                    ...
                    AND TRUNC(IT.FEC_CRE) >= TO_DATE('" & desde & "', 'DD/MM/YYYY')  
                    AND TRUNC(IT.FEC_CRE) <= TO_DATE('" & hasta & "', 'DD/MM/YYYY')
                    AND IT.ID_PROVEEDOR=" & idProveedor & "
                    GROUP BY PR_PRODUCTOS.CODIGO_BARRAS, PR_PRODUCTOS.DESCRIPCION, CP_PROVEEDORES.DENOMINACION
                ) ORDER BY CANTIDAD_VENDIDA DESC, PROVEEDOR ASC"
        ...
        If dt.Rows.Count > 0 Then
            ...
            txtProveedorIt.Text = CStr(dt.Rows(0).Item(2))
    ...
    Private Sub btnImprimir_Click(sender As Object, e As EventArgs) Handles btnImprimir.Click
        ...
        Dim report As New RptVentasPorFamilia
    ...
    Private Sub gridVentas_DoubleClick(sender As Object, e As EventArgs) Handles gridVentas.DoubleClick
        ...
        Dim idProveedor As Integer = GridView1.GetRowCellValue(rowhandle, "ID_PROVEEDOR")
    ...
    Private Sub btnImprimirIt_Click(sender As Object, e As EventArgs) Handles btnImprimirIt.Click
        ...
        proveedor = txtProveedorIt.Text
        desde = txtFechaDesdeIt.Text
        hasta = txtFechaHastaIt.Text

        Dim report As New RptItemsVentasPorFamilia
